...
void Viewer::timerEvent(QTimerEvent *)
{
    ...
    ::glBufferData(GL_ARRAY_BUFFER, sizeof(float) * x.size(), x.data(), GL_STATIC_DRAW);
...
void Viewer::init()
{
    ...
    for (int i = 0, c = 0; i < out.numberofedges; ++i) {
    ...
    ::glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned) * voronoi_edges.size(), voronoi_edges.data(), GL_STATIC_DRAW);
    ::glBindBuffer(GL_ARRAY_BUFFER, vbo[3]);
    ::glBufferData(GL_ARRAY_BUFFER, sizeof(float) * voronoi_ver.size(), voronoi_ver.data(), GL_STATIC_DRAW);
    ::glVertexPointer(3, GL_FLOAT, 0, 0);
    ::glBindBuffer(GL_ARRAY_BUFFER, 0);
    ::glBindVertexArray(0);
    n_voronoi_elements = voronoi_edges.size();
